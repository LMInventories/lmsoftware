...
async function load() {
  loading.value = true
  try {
    const res = await api.getHeadsUpSections()
    sections.value = (res.data || []).map(normalise)
    if (sections.value.length > 0) expanded.value[0] = true
  } catch (e) {
    console.error(e); alert('Failed to load Heads-Up sections')
  } finally {
    loading.value = false
  }
}
...
async function save() {
  saving.value = true
  try {
    const res = await api.updateHeadsUpSections(sections.value)
    sections.value = (res.data || []).map(normalise)
    dirty.value = false
  } catch (e) {
...
function addSection() {
  ...
  if (!name) return
  sections.value.push({ name, enabled: true, columns: ['name', 'answer', 'additional_notes'], items: [] })
...
<template>
  <div class="fs-wrap">
    ...
    <div class="fs-header">
      <div>
        <h2 class="fs-title">Heads-Up Report Sections</h2>
        <p class="fs-subtitle">
          These sections appear on <strong>Heads-Up Reports</strong> only.
          Build Yes / No / N/A question sets that give agents an instant snapshot of a property's condition.
        </p>
      </div>
    ...
    <div v-else>
      <div class="accordion">
        ...
        >
          <div class="acc-header" @click="toggleExpanded(sIdx)">
            ...
            <div class="sec-pills">
              <span v-for="colKey in section.columns" :key="colKey" class="col-pill">
                {{ colDef(colKey)?.label }}
              </span>
            </div>
          ...
          <div v-if="expanded[sIdx]" class="acc-body">
            <div class="col-config-bar">
              ...
              <div class="active-cols">
                <div v-for="(colKey, cIdx) in section.columns" :key="colKey" class="active-col-chip">
            ...
            <div class="items-table-wrap">
              <table v-if="section.items.length > 0" class="items-table">
                <thead>
                  <tr>
                    <th v-for="colKey in section.columns" :key="colKey">{{ colDef(colKey)?.label }}</th>
      ...
      <p class="fs-hint">
        Disabled sections are hidden from Heads-Up Reports but not deleted. Changes apply system-wide on save.
      </p>
    </div>
  </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          >
            ...
            <select v-if="colDef(colKey)?.type === 'dropdown'" v-model="itemModal.form[colKey]">
...
.items-empty { text-align: center; color: #94a3b8; font-size: 13px; padding: 20px 0 10px; }
