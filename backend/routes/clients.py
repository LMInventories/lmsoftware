...
def update_client(client_id):
    ...
    if 'report_photo_settings' in data: client.report_photo_settings = data['report_photo_settings']

    # These columns may not exist in older deployments — only set if present in the model
    for field in ('report_header_text_color', 'report_body_text_color', 'report_orientation'):
        if field in data and hasattr(client, field):
