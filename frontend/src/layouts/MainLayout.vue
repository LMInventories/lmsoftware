...
function goChangePassword() { closeMenu(); router.push('/change-password') }

// Computed helpers — easier to read in template
const isAdminOrManager = () => authStore.isAdmin || authStore.isManager
</script>
  ...
  <div class="layout">
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        <!-- Clerk only (not typist) -->
        <router-link v-if="authStore.isAdmin || authStore.isManager || authStore.isClerk" to="/properties" class="nav-item">
          <span class="icon">🏢</span><span>Properties</span>
        </router-link>

        <!-- Admin / Manager only -->
        <router-link v-if="authStore.isAdmin || authStore.isManager" to="/clients" class="nav-item">
          <span class="icon">👥</span><span>Portfolio</span>
        </router-link>

        <router-link v-if="authStore.isAdmin || authStore.isManager" to="/users" class="nav-item">
          <span class="icon">👤</span><span>Users</span>
        </router-link>

        <div v-if="authStore.isAdmin || authStore.isManager" class="nav-divider"></div>

        <router-link v-if="authStore.isAdmin || authStore.isManager" to="/settings/templates" class="nav-item">
