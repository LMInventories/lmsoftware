...
// ── Column definitions (the 9 available types) ────────────────────────────────
const COLUMN_TYPES = [
...
async function load() {
  ...
  try {
    ...
    console.error(e); alert('Failed to load fixed sections')
  } finally {
    loading.value = false
  }
}
...
function toggleColumn(sectionIndex, colKey) {
  ...
  if (idx === -1) {
    ...
    if (cols.length === 1) return // must keep at least one column
    cols.splice(idx, 1)
    // Also strip that key from all items
...
<template>
  <div class="fs-wrap">
    ...
    <div class="fs-header">
      <div>
        ...
        <p class="fs-subtitle">
          These sections appear on <strong>every</strong> inspection report.
    ...
    <div v-else>
        ...
        >
          ...
          <div v-if="expanded[sIdx]" class="acc-body">
            ...
            <div class="col-config-bar">
              ...
              <!-- Active columns (draggable order) -->
      ...
      <p class="fs-hint">
        Disabled sections are hidden from reports but not deleted. Changes apply system-wide on save.
      </p>
    </div>
  </div>
  ...
  <!-- ── Item add / edit modal ─────────────────────────────────────────────── -->
  <Teleport to="body">
    <!-- Close col picker if open when clicking outside -->
