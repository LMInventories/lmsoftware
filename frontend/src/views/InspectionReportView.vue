...
const currentPhoto   = ref(null)   // base64 of property's current photo

// reportData structure:
//   reportData[sectionId][String(rowId)][field]       — template row data
//   reportData[sectionId]._hidden                     — Set of rowIds to hide (soft-removed)
//   reportData[sectionId]._extra                      — [{_eid, ...fields}] added rows
//   reportData[roomId][itemId]._subs                  — [{_sid, description, condition}] sub-items
//   reportData[roomId]._extra                         — [{_eid, label, description, condition}] added room items
...
// ── Hide/show template rows (soft remove) ─────────────────────────────
...
function unhideRow(sectionId, rowId) {
  const sid = sectionId, rid = String(rowId)
  if (!reportData.value[sid]?._hidden) return
  reportData.value[sid]._hidden = reportData.value[sid]._hidden.filter(r => r !== rid)
  unsaved.value = true
}

const hasHidden = (sectionId) => (reportData.value[sectionId]?._hidden?.length ?? 0) > 0

// ── Extra rows (added lines in fixed sections) ─────────────────────────
function getExtra(sectionId) {
  return reportData.value[sectionId]?._extra ?? []
}
...
// ── Room extra items (added room-level lines) ─────────────────────────
function getRoomExtra(roomId) {
  return reportData.value[roomId]?._extra ?? []
}
...
// ── Sub-items (inside a room item) ────────────────────────────────────
function getSubs(roomId, itemId) {
  return reportData.value[roomId]?.[itemId]?._subs ?? []
}
...
onMounted(() => {
  load()
  timer = setInterval(() => { if (unsaved.value && !saving.value) save(false) }, 90000)
})
onBeforeUnmount(() => clearInterval(timer))

// ── Navigation ─────────────────────────────────────────────────────────
async function exit() {
  if (unsaved.value) await save(false)
...
const typeLabel = computed(() => ({
  ...
  interim: 'Interim Inspection', inventory: 'Inventory',
})[inspection.value?.inspection_type] ?? '')

const savedTime = computed(() =>
  lastSaved.value
    ? lastSaved.value.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })
    : null
)
...
function sectionStarted(sectionId) {
  ...
  if (!d) return false
  return Object.values(d).some(row =>
    row && typeof row === 'object' && Object.values(row).some(v => v !== '' && v != null)
  )
}
      ...
      <div class="topbar-r">
        <div v-if="allSections.length" class="prog">
          <div class="prog-track">
            <div class="prog-fill" :style="{ width: `${Math.round(startedCount / allSections.length * 100)}%` }"></div>
          </div>
        ...
        <span v-if="saving" class="chip chip-saving">
          <svg class="spin-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 12a9 9 0 1 1-6.2-8.6"/></svg>
          Saving
        </span>
        <span v-else-if="savedTime && !unsaved" class="chip chip-saved">
    ...
    <div class="body">
      ...
      <nav class="sidebar">
        <div v-if="template">
          <div v-if="fixedSections.length" class="nav-grp">
            ...
            <button v-for="s in fixedSections" :key="s.id"
              class="nav-btn" :class="{ active: activeId === s.id }"
              @click="scrollTo(s.id)">
              <span class="dot" :class="{ done: sectionStarted(s.id) }"></span>
              {{ s.name }}
            </button>
          </div>
          <div v-if="rooms.length" class="nav-grp">
            ...
            <button v-for="r in rooms" :key="r.id"
              class="nav-btn" :class="{ active: activeId === r.id }"
              @click="scrollTo(r.id)">
              <span class="dot" :class="{ done: sectionStarted(r.id) }"></span>
              {{ r.name }}
            </button>
          </div>
        </div>
        <div v-else class="sidebar-warn">
          ...
          <p>No template assigned. Return to the inspection overview to assign one first.</p>
        </div>
      </nav>
      ...
      <main class="main" @scroll="onScroll">
        ...
        <div v-if="!template" class="empty-state">
          <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" stroke-width="1"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          <h3>No template assigned</h3>
          <p>Assign a template from the inspection overview, then return here to fill in the report.</p>
        ...
        <div v-else>
          ...
          <div v-for="sec in fixedSections" :key="sec.id" :id="`sec-${sec.id}`" class="card">
            <div class="card-hd">
              ...
              <div class="card-hd-right">
                <span v-if="hasHidden(sec.id)" class="hidden-badge" @click="sec._showHidden = !sec._showHidden">
                  {{ (reportData[sec.id]?._hidden?.length ?? 0) }} hidden
                </span>
            ...
            <div v-if="sec.type === 'condition_summary'">
              <table class="tbl">
                <thead><tr><th class="col-item">Item</th><th>Condition</th><th class="col-del"></th></tr></thead>
                <tbody>
                  <tr v-for="row in sec.rows" :key="row.id" v-show="!isHidden(sec.id, row.id)">
                    <td class="td-name">{{ row.name }}</td>
                    <td><input class="fld-input" type="text" placeholder="Describe condition…"
                      :value="get(sec.id, row.id, 'condition')"
                      @input="set(sec.id, row.id, 'condition', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="hideRow(sec.id, row.id)" title="Remove">×</button></td>
                  </tr>
                  <tr v-for="ex in getExtra(sec.id)" :key="ex._eid" class="extra-row">
                    <td><input class="fld-input" type="text" placeholder="Item name…"
                      :value="ex.name" @input="setExtraField(sec.id, ex._eid, 'name', $event.target.value)" /></td>
                    <td><input class="fld-input" type="text" placeholder="Describe condition…"
                      :value="ex.condition" @input="setExtraField(sec.id, ex._eid, 'condition', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="removeExtraRow(sec.id, ex._eid)">×</button></td>
                  </tr>
                </tbody>
              </table>
              <div class="add-row-bar">
                <button class="add-row-btn" @click="addExtraRow(sec.id, sec.type)">+ Add line</button>
              </div>
            </div>
            ...
            <div v-else-if="sec.type === 'cleaning_summary'">
              <table class="tbl">
                <thead><tr><th class="col-item">Area</th><th class="col-clean">Cleanliness</th><th>Notes</th><th class="col-del"></th></tr></thead>
                <tbody>
                  <tr v-for="row in sec.rows" :key="row.id" v-show="!isHidden(sec.id, row.id)">
                    <td class="td-name">{{ row.name }}</td>
                    <td>
                      <select class="fld-select" :value="get(sec.id, row.id, 'cleanliness')"
                        @change="set(sec.id, row.id, 'cleanliness', $event.target.value)">
                        <option value="">— Select —</option>
                        <option v-for="o in cleanlinessOpts" :key="o" :value="o">{{ o }}</option>
                      </select>
                    </td>
                    <td><input class="fld-input" type="text" placeholder="Notes…"
                      :value="get(sec.id, row.id, 'cleanlinessNotes')"
                      @input="set(sec.id, row.id, 'cleanlinessNotes', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="hideRow(sec.id, row.id)" title="Remove">×</button></td>
                  </tr>
                  <tr v-for="ex in getExtra(sec.id)" :key="ex._eid" class="extra-row">
                    <td><input class="fld-input" type="text" placeholder="Area name…"
                      :value="ex.name" @input="setExtraField(sec.id, ex._eid, 'name', $event.target.value)" /></td>
                    <td>
                      <select class="fld-select" :value="ex.cleanliness"
                        @change="setExtraField(sec.id, ex._eid, 'cleanliness', $event.target.value)">
                        <option value="">— Select —</option>
                        <option v-for="o in cleanlinessOpts" :key="o" :value="o">{{ o }}</option>
                      </select>
                    </td>
                    <td><input class="fld-input" type="text" placeholder="Notes…"
                      :value="ex.cleanlinessNotes" @input="setExtraField(sec.id, ex._eid, 'cleanlinessNotes', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="removeExtraRow(sec.id, ex._eid)">×</button></td>
                  </tr>
                </tbody>
              </table>
            ...
            <div v-else-if="sec.type === 'smoke_alarms'">
              <div v-for="row in sec.rows" :key="row.id" v-show="!isHidden(sec.id, row.id)" class="qa-row">
                <div class="qa-row-header">
                  <p class="qa-question">{{ row.question }}</p>
                  <button class="del-btn" @click="hideRow(sec.id, row.id)" title="Remove">×</button>
                </div>
                <p v-if="row.notes" class="qa-guidance">{{ row.notes }}</p>
                <div class="qa-controls">
                  <div class="yn-group">
                    <label v-for="opt in ['Yes', 'No', 'N/A']" :key="opt" class="yn-chip"
                      :class="{ 'yn-yes': get(sec.id, row.id, 'answer')==='Yes' && opt==='Yes', 'yn-no': get(sec.id, row.id, 'answer')==='No' && opt==='No', 'yn-na': get(sec.id, row.id, 'answer')==='N/A' && opt==='N/A' }">
                      <input type="radio" :name="`${sec.id}_${row.id}`" style="display:none"
                        :checked="get(sec.id, row.id, 'answer') === opt"
                        @change="set(sec.id, row.id, 'answer', opt)" />{{ opt }}
                    </label>
                  </div>
                  <input class="fld-input fld-grow" type="text" placeholder="Additional notes…"
              ...
              <div v-for="ex in getExtra(sec.id)" :key="ex._eid" class="qa-row extra-row">
                <div class="qa-extra-header">
                  <input class="fld-input fld-grow" type="text" placeholder="Question / item…"
                    :value="ex.question" @input="setExtraField(sec.id, ex._eid, 'question', $event.target.value)" />
                  <button class="del-btn" @click="removeExtraRow(sec.id, ex._eid)">×</button>
                </div>
                <div class="qa-controls">
                  <div class="yn-group">
                    <label v-for="opt in ['Yes', 'No', 'N/A']" :key="opt" class="yn-chip"
                      :class="{ 'yn-yes': ex.answer==='Yes' && opt==='Yes', 'yn-no': ex.answer==='No' && opt==='No', 'yn-na': ex.answer==='N/A' && opt==='N/A' }">
                      <input type="radio" :name="`${sec.id}_${ex._eid}`" style="display:none"
                        :checked="ex.answer === opt" @change="setExtraField(sec.id, ex._eid, 'answer', opt)" />{{ opt }}
                    </label>
                  </div>
                  <input class="fld-input fld-grow" type="text" placeholder="Notes…"
                    :value="ex.notes" @input="setExtraField(sec.id, ex._eid, 'notes', $event.target.value)" />
                </div>
              </div>
              <div class="add-row-bar"><button class="add-row-btn" @click="addExtraRow(sec.id, sec.type)">+ Add line</button></div>
            </div>
            ...
            <div v-else-if="sec.type === 'fire_door_safety'">
              <div v-for="row in sec.rows" :key="row.id" v-show="!isHidden(sec.id, row.id)" class="qa-row">
                <div class="qa-row-header">
                  <div class="qa-location-badge">{{ row.name }}</div>
                  <button class="del-btn" @click="hideRow(sec.id, row.id)" title="Remove">×</button>
                </div>
                <p class="qa-question">{{ row.question }}</p>
                <p v-if="row.notes" class="qa-guidance">{{ row.notes }}</p>
              ...
              <div v-for="ex in getExtra(sec.id)" :key="ex._eid" class="qa-row extra-row">
                <div class="qa-extra-header">
                  <input class="fld-input" type="text" placeholder="Location…" style="width:160px"
                    :value="ex.name" @input="setExtraField(sec.id, ex._eid, 'name', $event.target.value)" />
                  <input class="fld-input fld-grow" type="text" placeholder="Question…"
                    :value="ex.question" @input="setExtraField(sec.id, ex._eid, 'question', $event.target.value)" />
                  <button class="del-btn" @click="removeExtraRow(sec.id, ex._eid)">×</button>
                </div>
                <div class="qa-controls">
            ...
            <div v-else-if="sec.type === 'keys'">
              <table class="tbl">
                <thead><tr><th class="col-item">Key / Access Item</th><th>Description</th><th class="col-del"></th></tr></thead>
                <tbody>
                  <tr v-for="row in sec.rows" :key="row.id" v-show="!isHidden(sec.id, row.id)">
                    <td class="td-name">{{ row.name }}</td>
                    <td><input class="fld-input" type="text" placeholder="e.g. 2× Yale front door key…"
                      :value="get(sec.id, row.id, 'description')"
                      @input="set(sec.id, row.id, 'description', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="hideRow(sec.id, row.id)" title="Remove">×</button></td>
                  </tr>
                  <tr v-for="ex in getExtra(sec.id)" :key="ex._eid" class="extra-row">
                    <td><input class="fld-input" type="text" placeholder="Key / item name…"
                      :value="ex.name" @input="setExtraField(sec.id, ex._eid, 'name', $event.target.value)" /></td>
                    <td><input class="fld-input" type="text" placeholder="Description…"
                      :value="ex.description" @input="setExtraField(sec.id, ex._eid, 'description', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="removeExtraRow(sec.id, ex._eid)">×</button></td>
                  </tr>
                </tbody>
              </table>
            ...
            <div v-else-if="sec.type === 'meter_readings'">
              <table class="tbl">
                <thead><tr><th class="col-item">Meter</th><th>Location &amp; Serial No.</th><th class="col-reading">Reading</th><th class="col-del"></th></tr></thead>
                <tbody>
                  <tr v-for="row in sec.rows" :key="row.id" v-show="!isHidden(sec.id, row.id)">
                    <td class="td-name">{{ row.name }}</td>
                    <td><input class="fld-input" type="text" placeholder="e.g. Understairs cupboard — SN 123456"
                      :value="get(sec.id, row.id, 'locationSerial')"
                      @input="set(sec.id, row.id, 'locationSerial', $event.target.value)" /></td>
                    <td><input class="fld-input fld-mono" type="text" placeholder="e.g. 12345.6"
                      :value="get(sec.id, row.id, 'reading')"
                      @input="set(sec.id, row.id, 'reading', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="hideRow(sec.id, row.id)" title="Remove">×</button></td>
                  </tr>
                  <tr v-for="ex in getExtra(sec.id)" :key="ex._eid" class="extra-row">
                    <td><input class="fld-input" type="text" placeholder="Meter name…"
                      :value="ex.name" @input="setExtraField(sec.id, ex._eid, 'name', $event.target.value)" /></td>
                    <td><input class="fld-input" type="text" placeholder="Location & serial…"
                      :value="ex.locationSerial" @input="setExtraField(sec.id, ex._eid, 'locationSerial', $event.target.value)" /></td>
                    <td><input class="fld-input fld-mono" type="text" placeholder="Reading…"
                      :value="ex.reading" @input="setExtraField(sec.id, ex._eid, 'reading', $event.target.value)" /></td>
                    <td class="td-del"><button class="del-btn" @click="removeExtraRow(sec.id, ex._eid)">×</button></td>
                  </tr>
                </tbody>
              </table>
          ...
          <div v-for="room in rooms" :key="room.id" :id="`sec-${room.id}`" class="card card-room">
            ...
            <!-- Template items -->
            <div v-for="item in room.sections" :key="item.id" class="room-row">
              <div class="room-row-label">{{ item.label }}</div>
              <div class="room-row-right">
                <div class="room-row-fields">
                  ...
                  <div v-if="item.hasCondition" class="room-field-cond">
                    ...
                    <textarea class="fld-textarea" rows="3"
                      :placeholder="`Condition of ${item.label.toLowerCase()}…`"
                ...
                <!-- Sub-items -->
                <div v-if="getSubs(room.id, item.id).length" class="sub-items">
                  <div v-for="sub in getSubs(room.id, item.id)" :key="sub._sid" class="sub-item">
                    <div class="sub-item-fields">
                      <div class="room-field-desc">
                        <label class="field-lbl">Description</label>
                        <textarea class="fld-textarea" rows="2" placeholder="Describe…"
                          :value="sub.description"
                          @input="setSubField(room.id, item.id, sub._sid, 'description', $event.target.value)"></textarea>
                      </div>
                      <div class="room-field-cond">
                        <label class="field-lbl">Condition</label>
                        <textarea class="fld-textarea" rows="2" placeholder="Condition…"
                          :value="sub.condition"
                          @input="setSubField(room.id, item.id, sub._sid, 'condition', $event.target.value)"></textarea>
                      </div>
                    </div>
                    <button class="del-btn del-btn-sub" @click="removeSubItem(room.id, item.id, sub._sid)" title="Remove">×</button>
                  </div>
                </div>
                <div class="sub-add-bar">
                  <button class="add-sub-btn" @click="addSubItem(room.id, item.id)">+ Add sub-item</button>
                </div>
              </div>
            </div>

            <!-- Extra room items (added lines) -->
            <div v-for="ex in getRoomExtra(room.id)" :key="ex._eid" class="room-row room-row-extra">
              <div class="room-row-label room-row-label-input">
                <input class="fld-input" type="text" placeholder="Item name…"
                  :value="ex.label"
                  @input="setRoomExtraField(room.id, ex._eid, 'label', $event.target.value)" />
              </div>
              <div class="room-row-right">
                <div class="room-row-fields">
                  <div class="room-field-desc">
                    ...
                    <textarea class="fld-textarea" rows="3" placeholder="Describe…"
                      ...
                      @input="setRoomExtraField(room.id, ex._eid, 'description', $event.target.value)"></textarea>
                  </div>
                  <div class="room-field-cond">
                    <label class="field-lbl">Condition</label>
                    <textarea class="fld-textarea" rows="3" placeholder="Condition…"
                      :value="ex.condition"
                      @input="setRoomExtraField(room.id, ex._eid, 'condition', $event.target.value)"></textarea>
                  </div>
                </div>
                <div class="sub-add-bar">
                  <button class="del-btn-text" @click="removeRoomExtraItem(room.id, ex._eid)">× Remove item</button>
                </div>
              </div>
            </div>
          ...
          <div class="foot">
            ...
            <button class="btn-save-lg" :disabled="saving" @click="save()">
      ...
      <div class="photo-modal">
        ...
        <div class="photo-modal-body">
          ...
          <div v-else class="photo-dropzone">
            ...
            <p class="photo-hint">Appears on the report cover page</p>
          </div>
          ...
          <p class="photo-hint-sm">JPG, PNG, WEBP — max 8MB. Will be displayed at 600px wide on the cover.</p>
        </div>

        <div class="photo-modal-ft">
          <button class="btn-ghost" @click="showPhotoModal = false">Cancel</button>
          <button class="btn-save-lg" :disabled="photoUploading || !currentPhoto" @click="savePhoto">
...
.crumb-type { font-size: 12px; font-weight: 700; color: #818cf8; white-space: nowrap; }
.crumb-who  { font-size: 12px; color: #475569; white-space: nowrap; }
.prog { display: flex; align-items: center; gap: 8px; }
.prog-track { width: 72px; height: 3px; background: rgba(255,255,255,0.08); border-radius: 2px; overflow: hidden; }
.prog-fill  { height: 100%; background: #6366f1; border-radius: 2px; transition: width 0.5s; }
.prog-lbl   { font-size: 11px; font-weight: 700; color: #475569; }
.chip { display: flex; align-items: center; gap: 5px; font-size: 11.5px; font-weight: 600; }
.chip-saving { color: #64748b; } .chip-saved { color: #4ade80; } .chip-unsaved { color: #fb923c; }
...
.save-btn:disabled { background: #1e3a5f; color: #475569; cursor: not-allowed; }
...
.nav-lbl { padding: 12px 14px 5px; font-size: 9.5px; font-weight: 800; text-transform: uppercase; letter-spacing: 1.2px; color: #2d4a6b; }
...
.nav-btn:hover { background: rgba(255,255,255,0.03); color: #64748b; }
.nav-btn.active { background: rgba(99,102,241,0.1); color: #a5b4fc; border-left-color: #6366f1; font-weight: 600; }
.dot { width: 7px; height: 7px; border-radius: 50%; background: transparent; border: 1.5px solid #2d4a6b; flex-shrink: 0; transition: all 0.2s; }
.dot.done { background: #22c55e; border-color: #22c55e; }
.sidebar-warn { padding: 20px 14px; display: flex; flex-direction: column; align-items: flex-start; gap: 10px; }
.sidebar-warn p { font-size: 12px; color: #4b6282; line-height: 1.5; }
...
.empty-state h3 { font-size: 20px; font-weight: 600; color: #1e293b; }
.empty-state p  { font-size: 14px; color: #64748b; max-width: 400px; line-height: 1.6; }
...
.card-title { font-size: 14px; font-weight: 700; color: #0f172a; letter-spacing: -0.1px; }
.card-hd-room .card-title { color: #4c1d95; }
.card-hint   { font-size: 11px; color: #94a3b8; }
.hidden-badge { font-size: 11px; color: #f59e0b; background: #fffbeb; border: 1px solid #fde68a; border-radius: 4px; padding: 2px 7px; cursor: pointer; }
...
.col-item { width: 22%; min-width: 140px; } .col-clean { width: 300px; } .col-reading { width: 160px; } .col-del { width: 36px; }
.td-name { font-size: 13px; font-weight: 600; color: #374151; white-space: nowrap; }
.td-del  { text-align: center; }
.extra-row td { background: #fafbff; }
...
.add-row-btn-room { border-color: #a78bfa; color: #7c3aed; }
.add-row-btn-room:hover { background: #f5f3ff; }

/* ── Delete buttons ────────────────────────────────────────────────── */
.del-btn { width: 24px; height: 24px; background: none; border: 1px solid #fca5a5; border-radius: 4px; font-size: 14px; line-height: 1; color: #ef4444; cursor: pointer; transition: all 0.12s; display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; }
.del-btn:hover { background: #fef2f2; border-color: #ef4444; }
.del-btn-sub { margin-top: 20px; }
.del-btn-text { background: none; border: none; color: #ef4444; font-size: 12px; font-weight: 600; cursor: pointer; padding: 2px 0; }
.del-btn-text:hover { text-decoration: underline; }
...
.room-row-extra { background: #faf8ff; }
.room-row-label { display: flex; align-items: flex-start; padding: 14px 16px; font-size: 13px; font-weight: 700; color: #374151; border-right: 1px solid #f1f5f9; background: #fafbfd; line-height: 1.3; }
.room-row-extra .room-row-label { background: #f0eeff; }
.room-row-label-input { padding: 10px 12px; align-items: center; }
.room-row-label-input .fld-input { font-weight: 700; font-size: 13px; }
...
.sub-add-bar { padding: 8px 16px; border-top: 1px dashed #e5e7eb; background: #fafbfd; }
...
.fld-mono { font-family: 'Courier New', monospace; font-weight: 700; letter-spacing: 1px; }
...
.btn-save-lg:hover:not(:disabled) { background: #4f46e5; }
.btn-save-lg:disabled { background: #94a3b8; cursor: not-allowed; }
